...
class FasterRCNNFPNMetaArch(model.DetectionModel):
  """Faster R-CNN Meta-architecture definition."""
  ...
  def __init__(self,
    ...
    # in the future.
    super(FasterRCNNFPNMetaArch, self).__init__(num_classes=num_classes)
  ...
  def _box_prediction(self, rpn_features_to_crop, proposal_boxes_normalized,
    ...
    flattened_proposal_feature_maps = (
        self._compute_second_stage_input_feature_maps(
            rpn_features_to_crop, proposal_boxes_normalized, true_image_shapes))
  ...
  def _extract_rpn_feature_maps(self, preprocessed_inputs):
    ...
    rpn_features_to_crop_list, self.endpoints = self._extract_proposal_features(
        preprocessed_inputs)

    feature_map_spatial_dims = self._get_feature_map_spatial_dims(rpn_features_to_crop_list)

    anchors_list = self._first_stage_anchor_generator.generate(
        feature_map_spatial_dims,
        im_height=image_shape[1],
        im_width=image_shape[2])
    anchors_list = box_list_ops.concatenate(anchors_list)

    rpn_box_predictor_features_list = \
         [self._first_stage_box_predictor_first_conv(feature_map) for feature_map in rpn_features_to_crop_list]

    return (rpn_box_predictor_features_list, rpn_features_to_crop_list,
            anchors_list, image_shape)
      ...
      if 'rpn_features_to_crop' in prediction_dict and self._initial_crop_size:
        detections_dict[
            'detection_features'] = self._add_detection_features_output_node(
                ...
                prediction_dict['rpn_features_to_crop'], true_image_shapes)
  ...
  def _add_detection_features_output_node(self, detection_boxes,
                                          rpn_features_to_crop, true_image_shapes):
    ...
    with tf.name_scope('SecondStageDetectionFeaturesExtract'):
      flattened_detected_feature_maps = (
          self._compute_second_stage_input_feature_maps(
              rpn_features_to_crop, detection_boxes, true_image_shapes))
  ...
  def _compute_second_stage_input_feature_maps(self, features_to_crop,
                                               proposal_boxes_normalized, true_image_shapes):
    ...
    levels = self._assign_levels(proposal_boxes_normalized, true_image_shapes)

    for level_i in range(3, 7+1):
        ...
        level_i_rois = tf.gather(proposal_boxes_normalized, level_i_indices, axis=1)

        cropped_regions = self._flatten_first_two_dimensions(
            self._crop_and_resize_fn(
                features_to_crop[level_i-3], level_i_rois,
                [self._initial_crop_size, self._initial_crop_size]))

        cropped_regions_list.append(cropped_regions)

    # TODO: Fix to allow batch size
    cropped_regions = tf.concat(cropped_regions_list, axis=0)
    num_proposals = tf.shape(proposal_boxes_normalized)[1]
    cropped_regions = tf.reshape(cropped_regions, [num_proposals] + cropped_regions.get_shape().as_list()[1:])

    return self._maxpool_layer(cropped_regions)


  def _assign_levels(self, proposal_boxes_normalized, true_image_shapes):
      boxlist = box_list.BoxList(tf.squeeze(proposal_boxes_normalized))
      boxlist_abs = box_list_ops.to_absolute_coordinates(boxlist, true_image_shapes[0, 0], true_image_shapes[0, 1])
      heights, widths = box_list_ops.height_width(boxlist_abs)

      # TODO: Revise formula 224
      levels = tf.floor(4. + tf.log(tf.sqrt(heights * widths + 1e-8) / 100.0) / tf.log(2.))  # 4 + log_2(***)

      # TODO: Configure by parameter
      min_level = int(3)  # self._feature_extractor.fpn_min_level
      max_level = int(7)  # min(5, int(cfgs.LEVLES[-1][-1]))
