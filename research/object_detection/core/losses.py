...
class SigmoidReducedFocalClassificationLoss(Loss):
  """Sigmoid reduced focal cross entropy loss.

  Reduced focal loss down-weights well classified examples and focusses on the hard
  examples. See https://arxiv.org/pdf/1903.01347.pdf for the loss definition.
  """

  def __init__(self, gamma=2.0, threshold=0.5, alpha=0.25):
    """Constructor.
    ...
    """
    super(SigmoidReducedFocalClassificationLoss, self).__init__()
    self._alpha = alpha
  ...
  def _compute_loss(self,
    ...
    if self._gamma:
      modulating_factor = tf.cast(tf.less(p_t, self._threshold), p_t.dtype)
      threshold_mask = tf.cast(tf.greater_equal(p_t, self._threshold), p_t.dtype)
      modulating_factor = modulating_factor + tf.pow(1.0 - p_t, self._gamma) * threshold_mask
...
class SoftmaxReducedFocalClassificationLoss(Loss):
  """Softmax reduced focal cross entropy loss.
  ...
  def __init__(self, gamma=2.0, threshold=0.5, alpha=0.25):
    ...
    Args:
      ...
      alpha: optional alpha weighting factor to balance positives vs negatives.
    """
    super(SoftmaxReducedFocalClassificationLoss, self).__init__()
    self._alpha = alpha
    self._threshold = threshold
    self._gamma = gamma
  ...
  def _compute_loss(self,
    ...
    if class_indices is not None:
      weights *= tf.reshape(
          ...
          [1, 1, -1])
    epsilon = 1.e-9
    p_t = tf.add(tf.nn.softmax(prediction_tensor), epsilon)
    per_entry_cross_ent = tf.multiply(target_tensor, -tf.log(p_t))
    modulating_factor = 1.0
    if self._gamma:
      modulating_factor = tf.cast(tf.less(p_t, self._threshold), p_t.dtype)
      threshold_mask = tf.cast(tf.greater_equal(p_t, self._threshold), p_t.dtype)
